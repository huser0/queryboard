...
fn decimal_with_precision_and_scale_roundtrips_exactly() {
    ...
    assert_eq!(
        value,
        CellValue::Decimal("12345678901234.1234".to_string())
    );
    });
}
...
fn bigint_at_i64_boundary() {
    RUNTIME.block_on(async {
    let mut session = session().await;
    let value = select_one_cell(&mut *session, "SELECT CAST(9223372036854775807 AS SIGNED)").await;
...
fn bool_true_and_false() {
    ...
    let mut session = session().await;
    assert_eq!(
        select_one_cell(&mut *session, "SELECT active FROM bind_repro WHERE id = 8002").await,
        CellValue::Bool(true)
    );
    });
}
...
fn null_in_every_mapped_type() {
    ...
    for cast in [
        ...
        assert_eq!(value, CellValue::Null, "CAST(NULL AS {cast}) deveria virar Null");
    }
    });
}
...
fn text_bind_matches_typed_columns() {
    ...
    let cases: &[(&str, &str)] = &[
        ...
        ("SELECT id FROM bind_repro WHERE sale_date = ?", "2026-07-20"),
    ];
